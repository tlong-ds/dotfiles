...
do
  ...
  -- Expose minimal RPC helpers so shell panes can sync cwd into specific tabs.
  _G.ShellSync = _G.ShellSync or {}

  ---@param tabnr integer|string
  ---@return integer
  function _G.ShellSync.current_tabnr(tabnr)
    if tabnr ~= nil then return tonumber(tabnr) or 0 end
  ...
  ---@param path string
  ---@return integer
  function _G.ShellSync.set_tab_cwd(tabnr, path)
    local target_nr = tonumber(tabnr)
    if not target_nr or type(path) ~= 'string' or vim.fn.isdirectory(path) ~= 1 then return 0 end

    local tabs = vim.api.nvim_list_tabpages()
    local target = tabs[target_nr]
    ...
    -- Force the tabline and statusline to redraw immediately.
    -- Without this, the UI stays stale until the user presses a key,
    -- because RPC-triggered changes don't cause an automatic screen refresh.
    if ok then
  ...
  -- Single entry point called by the shell's chpwd hook.
  -- Changes the current tab's working directory and immediately redraws.
  ---@param path string
  ---@return integer 1 on success, 0 on failure
  function _G.ShellSync.sync_cwd(path)
    if type(path) ~= 'string' or vim.fn.isdirectory(path) ~= 1 then return 0 end
    local ok = pcall(vim.cmd, 'tcd ' .. vim.fn.fnameescape(path))
    if ok then
      vim.schedule(function()
        vim.cmd 'redraw!'
      end)
  ...
  ---@return string
  function _G.ShellSync.tab_snapshot()
    local rows = {}
    for i, tab in ipairs(vim.api.nvim_list_tabpages()) do
      local ok_cwd, cwd = pcall(vim.fn.getcwd, -1, i)
  ...
  ---@return string
  function _G.ShellSync.current_tab_cwd_label()
    local cwd = vim.fn.getcwd(-1, vim.fn.tabpagenr())
    return vim.fn.fnamemodify(cwd, ':t')
  end

  ---@return string
  function _G.ShellSync.render_tabline()
    local parts = {}
    ...
    for i, tab in ipairs(vim.api.nvim_list_tabpages()) do
      ...
      local buf = vim.api.nvim_win_get_buf(win)
      local name = vim.api.nvim_buf_get_name(buf)
      local label = ok_cwd and vim.fn.fnamemodify(cwd, ':t') or '?'
  ...
  -- Ensure common Homebrew binaries are visible inside Neovim, including GUI launches.
  ...
  for _, entry in ipairs(path_entries) do
    path_index[entry] = true
  end
  for _, entry in ipairs { '/opt/homebrew/bin', '/usr/local/bin' } do
    if vim.fn.isdirectory(entry) == 1 and not path_index[entry] then
      table.insert(path_entries, 1, entry)
      path_index[entry] = true
    end
  end
  vim.env.PATH = table.concat(path_entries, path_sep)
...
---@param repo string
---@return string
local function gh(repo) return repo end
...
require('lazy').setup({
  ...
  { gh 'nvim-tree/nvim-web-devicons' },
  {
    ...
    cmd = 'Neotree',
    keys = {
      { '<C-n>', '<cmd>Neotree filesystem reveal left<CR>', desc = 'Open file tree' },
      { '\\', '<cmd>Neotree filesystem reveal left<CR>', desc = 'Open file tree' },
    },
    opts = {
  ...
  { gh 'folke/tokyonight.nvim', priority = 1000 },
...
do
  ...
  --
  -- Plugins are declared in the `lazy.nvim` spec above.
  -- They are installed automatically on first run, and can also be managed via `:Lazy`.
  --
  ...
  require('gitsigns').setup {
    ...
    on_attach = function(bufnr)
      ...
      map({ 'o', 'x' }, 'ih', gitsigns.select_hunk, 'Select git [h]unk')
    end,
  }
  ...
  vim.o.showtabline = 2
  vim.o.tabline = '%!v:lua.ShellSync.render_tabline()'
  ...
  statusline.section_filename = function()
    local file = '%f%m%r'
    local cwd = _G.ShellSync.current_tab_cwd_label()
    return string.format('%s [t%d:%s]', file, vim.fn.tabpagenr(), cwd)
  end
...
do
  ...
  vim.keymap.set('n', '<D-F>', builtin.find_files, { desc = 'Search [F]iles' })
  ...
  vim.api.nvim_create_autocmd({ 'BufEnter', 'BufWritePost', 'InsertLeave' }, {
    ...
    callback = function()
      if vim.bo.modifiable then
        ...
        local active_linters = lint.linters_by_ft[ft]
        if active_linters then
...
-- ============================================================
-- SECTION 5B: DEBUGGING
-- nvim-dap, dap-ui, and Go debugger setup
-- ============================================================
do
